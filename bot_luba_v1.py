...
def gel_organic_kitchen(call):
    msg = '*Мягкий гель для умывания Don’t touch my face*.\n' \
          'Нежный гель для умывания бережно очищает кожу благодаря мягким моющим компонентам, оставляя ощущение ' \
          'комфорта и увлажненности. Органический сок алоэ и глицерин увлажняют и успокаивают кожу. ' \
          'Органический экстракт зеленого чая обладает противовоспалительным действием и улучшает цвет лица. ' \
          'Пантенол смягчает и снимает раздражения. Подходит для умывания и снятия нестойкого макияжа.\n' \
          '*Цена*: объём 170мл - 357₽.'
    photo = open('static/gel_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')
...
def krem_organic_kitchen(call):
    msg = '*Увлажняющий успокаивающий крем "Don’t touch my face" для склонной к сухости кожи*.\n' \
          'Крем увлажняет и успокаивает нормальную и склонную к сухости кожу. Фосфолипиды, органические ' \
          'масла ши и авокадо смягчают, восполняют недостаток естественных липидов и восстанавливают защитный ' \
          'барьер. Ниацинамид успокаивает кожу и снимает раздражения, способствует снижению ее чувствительности. ' \
          'Гиалуроновая кислота увлажняет, убирает чувство стянутости, придает коже упругость и эластичность.\n' \
          '*Цена*: 357₽.'
    photo = open('static/krem_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')


#  2. Лёгкий крем для лица Organic Kitchen.
def legkiy_krem_dlya_lica_organic_kitchen(call):
    msg = '*Легкий увлажняющий успокаивающий крем "Don’t touch my face" для склонной к жирности кожи*.\n' \
          'Легкий крем увлажняет и успокаивает склонную к жирности кожу. Благодаря нежной текстуре крем ' \
          'прекрасно впитывается, не утяжеляя кожу и не оставляя ощущения жирности. Фосфолипиды, ' \
          'органическое масло шиповника и масло жожоба питают и восстанавливают защитный барьер. ' \
          'Ниацинамид успокаивает кожу и снимает раздражения, способствует уменьшению воспалений. ' \
          'Гиалуроновая кислота увлажняет, убирает чувство стянутости, придает коже упругость и эластичность.\n' \
          '*Цена*: 357₽.'
    photo = open('static/legkiy_krem_dlya_lica_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')
...
def maska_s_lifting_effectom_organic_kitchen(call):
    msg = '*Маска-лифтинг для лица "Укол красоты"*.\nСловно волшебный укол красоты маска-лифтинг ' \
          'преображает Вашу кожу, повышает упругость и эластичность, разглаживает мелкие морщинки, ' \
          'делает контур лица более правильным и чётким. Свежее яблоко подтягивает и тонизирует кожу, ' \
          'придает ей свежесть и здоровый вид. Органический ройбуш увлажняет и смягчает кожу, насыщает ' \
          'аминокислотами, делает ее более плотной и упругой.\n*Цена*: 119₽.'
    photo = open('static/maska_s_lifting_effectom_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')


#  2. Маска увлажняющая «Огуречные кружочки» Organic Kitchen.
def maska_uvlazhn_organic_kitchen(call):
    msg = '*Маска для лица Organic Shop "Огуречные кружочки"*.\nНужен мгновенный увлажняющий уход, ' \
          'как от огуречных кружочков? Новая маска в любое время года подарит коже максимальное увлажнение, ' \
          'надолго сохранит свежесть и красоту. Органический гель алоэ насыщает живительной влагой, делая ' \
          'кожу гладкой и сияющей. Свежий огурец и морские минералы глубоко увлажняют, тонизируют кожу и ' \
          'заряжают энергией. Напоите Вашу кожу огуречным фрешем, ей точно понравится.\n*Цена*: 129₽.'
    photo = open('static/maska_uvlazhn_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')


#  3. Кислотный пилинг Organic Kitchen.
def kislotniy_piling_organic_kitchen(call):
    msg = '*Натуральный био пилинг для идеального тона кожи "John LEMON"*.\nВсего несколько минут до важного ' \
          'события? Этот цитрусовый пилинг как по волшебству превратит тебя в звездную красотку! ' \
          'Органическое масло бразильского лимона тонизирует кожу и осветляет несовершенства тона, ' \
          'улучшая цвет лица. Цедра красного грейпфрута способствует сокращению пор и придает роскошное ' \
          'сияние. Фруктовые кислоты полируют, выравнивая рельеф и разглаживая морщинки.\n*Цена*: 140₽.'
    photo = open('static/kislotniy_piling_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')


#  4. Гоммаж  для лица «Дыня» Organic Kitchen.
def gommazh_dlya_lica_organic_kitchen(call):
    msg = '*Organic Kitchen Натуральный обновляющий БИО Гоммаж для лица*.\n' \
          'Натуральное ароматное средство для возрождения красоты вашей кожи. Органический экстракт дыни питает ' \
          'и увлажняет, запускает процессы обновления, восстанавливает упругость, разглаживает рельеф. ' \
          'Семена черного тмина очищают кожу, способствуют снятию воспалений, улучшают цвет лица. ' \
          'Тростниковый сахар нежно полирует, придает сияние и шелковистость.\n*Цена*: 140₽.'
    photo = open('static/gommazh_dlya_lica_organic_kitchen.png', 'rb')
    bot.send_photo(call.message.chat.id, photo, msg, parse_mode='Markdown')
